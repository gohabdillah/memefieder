...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Train local RandomForest model for pose-to-meme classification."
    )
    parser.add_argument(
        "--data-dir",
        type=str,
    ...
    parser.add_argument(
        ...
        help="Output path for trained local model pickle.",
    )
...
def fit_model(
    ...
    if not args.quick_grid_search:
        model = build_model(args)
        model.fit(x_train, y_train)
        return model, ""

    min_class_count = int(np.min(np.unique(y_train, return_counts=True)[1]))
    cv_folds = min(max(2, args.cv_folds), min_class_count)
    if cv_folds < 2:
        model = build_model(args)
        model.fit(x_train, y_train)
    ...
    summary = (
        ...
        f"Best {args.scoring}: {search.best_score_:.4f}, "
...
def train_model(
    ...
    if can_make_holdout_split(y):
        ...
        model, grid_summary = fit_model(x_train, y_train, args)
        y_pred = model.predict(x_test)
        split_scope = "holdout test split"
        ...
        return model, report, split_scope, cm, labels_in_data, grid_summary
...
def main() -> None:
    ...
    model, report, split_scope, cm, cm_labels, grid_summary = train_model(x, y, args)
    ...
    if args.confusion_matrix_csv:
        csv_path = Path(args.confusion_matrix_csv)
        save_confusion_matrix_csv(csv_path, cm, cm_labels)
